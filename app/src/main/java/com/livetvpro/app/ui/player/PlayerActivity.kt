...
class PlayerActivity : AppCompatActivity() {
    ...
    override fun onPictureInPictureModeChanged(
        ...
        controlsState.hide()
        binding.playerControlsCompose.alpha = 0f
    ...
    private fun exitPipUIMode(newConfig: Configuration) {
        ...
        binding.playerControlsCompose.alpha = 1f
    ...
    override fun onKeyDown(keyCode: Int, event: android.view.KeyEvent?): Boolean {
        ...
        return when (keyCode) {
            ...
            android.view.KeyEvent.KEYCODE_DPAD_RIGHT -> {
                player?.let { it.seekTo((it.currentPosition + skipMs).coerceAtMost(it.duration.coerceAtLeast(0L))) }
                controlsState.show(lifecycleScope)
                true
            }
            ...
            android.view.KeyEvent.KEYCODE_NUMPAD_ENTER -> {
                controlsState.toggle(lifecycleScope)
                true
            }
    ...
    fun updatePipParams(enter: Boolean = false): PictureInPictureParams {
        ...
        player?.videoFormat?.let { format ->
            ...
            if (width > 0 && height > 0) {
                ...
                if (rational.toFloat() in 0.42f..2.38f) {
                    builder.setAspectRatio(rational)
                    builder.setSourceRectHint(calculateSourceRect(rational))
                }
            }
        }
    ...
    private fun calculateSourceRect(aspectRatio: Rational): Rect {
        ...
        if (viewWidth == 0f || viewHeight == 0f) return Rect()
